 ...
 * <p>
 * This interface provides methods for generating, retrieving, validating, and removing JWTs,
 * as well as accessing the signing key used for token creation and verification. It is used
 * to support secure user authentication and session management, typically with JWTs stored
 * in HTTP-only cookies for client-server communication.
 * </p>
 *
 */
public interface JwtService {
   ...
   * <p>
   * This method creates a JWT based on the provided username, signs it with the application's
   * secret key, and attaches it to the response, typically as an HTTP-only cookie, to establish
   * a user session.
   * </p>
   *
   ...
   * <p>
   * This method extracts the JWT from the request's cookies, typically stored in an HTTP-only
   ...
   * <p>
   * This method verifies the JWT's signature, expiration, and other claims using the application's
   * secret key. If the token is invalid or expired, an exception is thrown.
   * </p>
   *
   ...
   * <p>
   * This method clears the JWT cookie, typically named 'jwt', from the client by setting its
   ...
   * <p>
   * This method returns the {@link SecretKey} used to sign and validate JWTs, typically derived
   * from a Base64-encoded secret configured in the application.
   * </p>
   *
   * @return the {@link SecretKey} for JWT signing and verification
   * @throws com.rookie.asset_management.exception.AppException if the secret key is invalid or
   *         not configured
   */
   ...
   * <p>
   * This method parses the JWT and retrieves the username stored in the token's subject claim.
