 ...
 *
 * <p>This class implements Spring Security's {@link AuthenticationEntryPoint} to define the
 * behavior when an unauthenticated user attempts to access a protected resource. It responds with
 * an HTTP 401 Unauthorized status and a descriptive error message, typically used in JWT-based
...
public class AuthEntryPointJwt implements AuthenticationEntryPoint {
   ...
   *
   * <p>This method is invoked when an {@link AuthenticationException} occurs, indicating that the
   * request lacks valid authentication credentials (e.g., a missing or invalid JWT token). It sets
   * the HTTP response status to 401 and includes an error message to inform the client of the
